...
const sendEventNotification = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      ...
      message:
        recipients.length > 0
          ? `Đã gửi thông báo cho ${recipients.length} người đăng ký`
          : "Đã tạo thông báo cho sự kiện",
      data: {
        notification: {
          title: newNotification.title,
          message: newNotification.message,
  ...
  try {
    ...
    for (const event of upcomingEvents) {
      ...
      const reminderNotification = {
        ...
        message: `Sự kiện "${
          event.title
        }" sẽ diễn ra vào ${event.start_time.toLocaleString("vi-VN")} tại ${
          event.location
...
const getAllNotificationsForStudents = async (req, res) => {
  try {
    const page = parseInt(req.query.page, 10) || 1;
    const limit = parseInt(req.query.limit, 10) || 10;
    const skip = (page - 1) * limit;
    ...
    // Lấy thông tin sự kiện cho mỗi thông báo
    const notificationsWithEventDetails = await Promise.all(
      notifications.map(async (notif) => {
        const event = await eventsCollection.findOne({ _id: notif.event_id });
        return {
          _id: notif._id.toString(),
          event_id: notif.event_id.toString(),
          event_title: event ? event.title : "Sự kiện không tồn tại",
          title: notif.title,
          message: notif.message,
          type: notif.type,
          sent_at: notif.sent_at.toISOString(),
    ...
    return res.status(200).json({
      success: true,
      data: {
        notifications: notificationsWithEventDetails,
        pagination: {
          page,
          pages,
          total,
    ...
    console.error("❌ Lỗi khi getAllNotificationsForStudents:", err);
    return res.status(500).json({
      success: false,
      message: "Lỗi server nội bộ",
    });
